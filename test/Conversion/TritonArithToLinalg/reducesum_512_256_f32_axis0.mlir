...
// CHECK:           %[[SPLAT_1:.*]] = tensor.splat %[[ARG1]] : tensor<256x!tt.ptr<f32>>
// CHECK:           %[[GENERIC_7:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_0]], #[[$ATTR_0]], #[[$ATTR_0]]], iterator_types = ["parallel"]} ins(%[[SPLAT_1]], %[[GENERIC_3]] : tensor<256x!tt.ptr<f32>>, tensor<256xi32>) outs(%[[SPLAT_1]] : tensor<256x!tt.ptr<f32>>) {
...
// CHECK:           %[[EMPTY_5:.*]] = tensor.empty() : tensor<256xf32>
// CHECK:           %[[FILL_1:.*]] = linalg.fill ins(%[[CONSTANT_0]] : f32) outs(%[[EMPTY_5]] : tensor<256xf32>) -> tensor<256xf32>
// CHECK:           %[[REDUCE_0:.*]] = linalg.reduce ins(%[[LOAD_0]] : tensor<512x256xf32>) outs(%[[FILL_1]] : tensor<256xf32>) dimensions = [0]
// CHECK:             (%[[VAL_18:.*]]: f32, %[[VAL_19:.*]]: f32) {
// CHECK:               %[[ADDF_0:.*]] = arith.addf %[[VAL_18]], %[[VAL_19]] : f32
// CHECK:               linalg.yield %[[ADDF_0]] : f32
// CHECK:             }
// CHECK:           tt.store %[[GENERIC_7]], %[[REDUCE_0]] : tensor<256x!tt.ptr<f32>>
// CHECK:           return
// CHECK:         }
