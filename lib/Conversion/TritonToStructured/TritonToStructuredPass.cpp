...
public:
  ...
  LogicalResult convertToPointerTupleWithOffsetsAndStrides() {
    ...
    converter.addConversion(
        [context](RankedTensorType tensorType) -> std::optional<Type> {
          // Important note:
          ...
          //
          // If std::nullopt is returned, the converter is allowed to try
          // another conversion function to perform the conversion.
          //
          // Say we have type tensor<4x256xbf16> which is a RankedTensorType.
          // Even though this RankedTensorType matches the converter that
          // handles the tuple conversion, we want to keep this type as is
          // because the inner type isn't a pointer.
          //
    ...
    converter.addConversion(
        [context](triton::PointerType ptrType) -> std::optional<Type> {
          return getStructuredStateTupleType(context, ptrType);
        });
